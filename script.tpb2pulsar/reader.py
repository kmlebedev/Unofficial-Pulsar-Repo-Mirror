    ...
    def __init__(self, movie=True, show=True):
        ...
        if movie:
            ...
            if self.movie_folder == '':
                self.dialog.ok('Warning!', 'Movie Folder cannot be empty.\nPlease, check the Download section in the settings')
                self.settings.openSettings()
        if show:
            self.show_folder = self.settings.getSetting('show_folder')
            if self.show_folder == '':
                self.dialog.ok('Warning!',
                               'Show Folder cannot be empty.\nPlease, check the Download section in the settings')
                self.settings.openSettings()
        # remove .strm
        self.number_pages = int('0%s' % self.settings.getSetting('number_pages'))
        self.dialog = xbmcgui.Dialog()
                ...
                if check:  # the file has passed the filtering
                    ...
                    if not os.path.exists(directory):
                        try:
                            ...
                            pass
                    if 'magnet' not in magnet[cm]:
                        link = 'plugin://script.rss2pulsar?uri=%s' % quote_plus(magnet[cm]) #  it is a torrent
                    elif plugin == 'Pulsar':
