...
def update_service():
    ...
    if settings.service == 'true':
        ...
        sort = settings.settings.getSetting('sort')
        number= settings.settings.getSetting('number')
        url_search = "%s/api/v2/list_movies.json?limit=%s&quality=%s&sort_by=%s&order_by=desc" % (settings.url_address, number, quality, sort.lower().replace(' ', '_'))
