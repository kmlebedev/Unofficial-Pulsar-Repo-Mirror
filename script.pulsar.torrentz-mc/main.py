...
def extract_torrents(data):
    try:
        ...
        for cm, (infohash, name) in enumerate(re.findall('<dl><dt><a href="/(.*?)">(.*?)<', data)):
            ...
            name = size[cm] + ' - ' + name.replace('-', ' ').title() + ' - ' + settings.name_provider #find name in the torrent
            if filters.verify(name,size[cm]):
                    results.append({"name": name, "uri":  torrent, "info_hash": infohash,
                           ...
                           "peers": int(peers[cm].replace(',', '')), "language": settings.language})  # return le torrent
                    cont+= 1
            else:
