...
def extract_torrents(data):
    try:
        ...
        for cm, (torrent,name) in enumerate(re.findall('/torrent/(.*?)/(.*?).html', data)):
            ...
            name = size[cm] + 'B' + ' - ' + name.replace('-', ' ').title() + ' - ' + settings.name_provider #find name in the torrent
...
def search(query):
    ...
    url_search = "%s/all/torrents/%s.html" % (settings.url, query)  # change in each provider
